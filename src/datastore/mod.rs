...
pub fn update_product_amount(product_name: String, amount: i32) -> Result<u64, Error> {
    ...
    let row = client.execute("UPDATE orders 
                SET amount = $2
                WHERE product_name = $1", &[&product_name, &amount],)?;
...
pub fn create_order(product_name: String, amount: i32) -> Result<(), Error> {
    ...
    let rows = client.query("SELECT amount FROM orders WHERE product_name = $1", &[&product_name])?;

    if rows.len() == 0 {
        client.execute("INSERT INTO orders (product_name, amount) VALUES ($1, $2)", &[&product_name, &amount],)?;
    }
